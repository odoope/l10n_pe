...
class ResPartner(models.Model):
    ...
    @api.model
    def ruc_connection(self, ruc):
        session = requests.Session()
        headers = requests.utils.default_headers()
        headers['User-Agent'] = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36'
        headers['Content-Type'] = 'application/json;chartset=utf-8'
        url_ruc_api = 'https://ruc.com.pe/api/v1/consultas'
        values_params = {"ruc":ruc,"token": "4a44f5a1-6fe0-4163-8483-a39dc08fae84-dcf517e0-de60-49ac-8286-47bc4d174403"}
        data = {}
        try:
            response = session.post(url=url_ruc_api,json=values_params,headers=headers,timeout=(15)).content
    ...
    def l10n_pe_ruc_connection(self, ruc):
        data = {}
        if self.env.user.company_id.l10n_pe_api_ruc_connection == 'consul_ruc_api':  
            data = self.ruc_connection(ruc)
        return data       
    
    
    @api.model     
    def dni_connection(self, dni):
        session = requests.Session()
        ...
        url_dni_api = 'https://ruc.com.pe/api/v1/consultas'
        values_params = {"dni":dni,"token": "4a44f5a1-6fe0-4163-8483-a39dc08fae84-dcf517e0-de60-49ac-8286-47bc4d174403"}
        data = {}        
        try:
            response = session.post(url=url_dni_api,json=values_params,headers=headers,timeout=(15)).content
            result = json.loads(response)
    ...
    def l10n_pe_dni_connection(self, dni):
        ...
        if company.l10n_pe_api_dni_connection == 'consul_dni_api':
            print("METODO ACTVADO",dni)
            data = self.dni_connection(dni)
        return data   
